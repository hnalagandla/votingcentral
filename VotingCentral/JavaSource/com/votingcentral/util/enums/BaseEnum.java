...
public abstract class BaseEnum implements Cloneable, Serializable {
	...
	private transient Integer m_integerId = null;
	...
	/**
	 * Base constructor to set the id and name of the constant. It should be
	 * called by other constructors from the derived classes. It internally adds
	 * this object into the EnumManager, which in turn sets the relationship
	 * between this constant with others, such as previous and next constants of
	 * the same type.
	 */
	...
	/**
	 * Supports the ability to create an Enum instance but NOT have it added to
	 * its EnumManager. This basically invalidates all the general capabilities
	 * and uniqueness assertion. This is useful to support dynamic creation of
	 * Enum types on the fly who manager their own identity as well as allow for
	 * testing.
	 * 
	 * Note that the registration would simply keep growing for these on-the-fly
	 * instances and finally blow our memory...
	 * 
	 * @param id
	 * @param name
	 * @param register
	...
	/**
	 * Return the int id value defined in this constant. It is temp method to
	 * keep compatable interface from the previous impl.
	 * 
	 * @return int - id of the constant.
	 */
	...
	/**
	 * Return the id value wrapped as an Integer. The returned value is cached
	 * and reused by subsequent calls to this method.
	 * 
	 * @return Integer - wrapper object for id of the constant.
	 */
	public final Integer getInteger() {
		if (m_integerId == null) {
			m_integerId = new Integer(m_id);
		}
		return m_integerId;
	}

	/**
	 * Returns true if the object is the same type as this one and the ids are
	 * equal.
	 * 
	 * @param other
	 *            an Object reference for comparison
	 * 
	...
	/**
	 * Return a List for a given class type. The order of the constants this
	 * List returned is based on the definition order of the constants for that
	 * type. The List may be modified and will not affect the contents of the
	 * Enum.
	 * 
	 * @param type
	 *            a Class type.
	...
	/**
	 * Return a existing constant for a given class type and id. If the type is
	 * not an Enum type and exception is thrown. If the id does not exist in the
	 * Enum, null is returned.
	 * 
	 * @param type
	 *            a Class type.
	 * @param id
	...
	/**
	 * Return the first existing constant for a given class type and name. If
	 * the type is not an Enum type and exception is thrown. If the id does not
	 * exist in the Enum, null is returned. Since an Enum is not gauranteed to
	 * have a unique name, the first match (if any) is returned.
	 * 
	 * @param type
	 *            a Class type.
	 * @param id
	 *            an String value for name.
	 * 
	 * @return BaseEnum - an existing constant, null if not exists.
	 */
	...
	/**
	 * Return a existing constant for a given class type and id. If not exists,
	 * return the passed-in default constant.
	 * 
	 * @param type
	 *            a Class type.
	 * @param id
	 *            an int value for id.
	 * @param defaultEnum
	 *            an alternative constant if can find existing one.
	 * 
	 * @return BaseEnum - an existing constant, defaultEnum if not exists.
	 */
	public static final BaseEnum getElseReturnEnum(Class type, int id,
			BaseEnum defaultEnum) {
	 ...
	 */
	protected static Object createEnclosedEnumArray(Class clz,
			Class[] enclosingClzz) {
